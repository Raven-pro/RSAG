...
export async function onRequestPost(context) {
    ...
    try {
        ...
        const publication = await db.prepare(`
            SELECT id, title, created_by, pdf_url
            FROM publications
            WHERE id = ?
        `).bind(id).first();
        ...
        await logActivity(
            db,
            isAdmin ? '绑定论文PDF' : '绑定论文PDF并提交审核',
            'publications',
            id,
            user.username,
            `${isAdmin ? '绑定论文PDF' : '绑定论文PDF并提交审核'}: ${publication.title} <- ${targetFile.original_name || 'PDF'}`
        );

        return createResponse({
            publicationId: id,
            url: fileUrl,
            message: isAdmin ? '论文 PDF 绑定成功' : '论文 PDF 已绑定并提交审核'
        });
    } catch (error) {
